...
int main(int argc, char** argv){
  ...
  pick_goal.target_pose.pose.position.y = -3.0;
  ...
  // Define a position and orientation for the robot to reach
  drop_goal.target_pose.pose.position.x = 1.4;
  drop_goal.target_pose.pose.position.y = 1.7;
  ...
  if(ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED){
    ROS_INFO("Hooray, robot able to pick object!!");
  	ros::Duration(5).sleep();
    ROS_INFO("Sending the drop location");
  	ac.sendGoal(drop_goal);
  	ac.waitForResult();
  		if(ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED){
      		ROS_INFO("Hooray, robot drop the object!!");
     	}
    else
    	{
    ...
    ROS_INFO("The base failed to move TO pick location for some reason");
